resource "aws_cloudfront_distribution" "hiteshshop" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.hiteshshop.domain_name
        zone_id = aws_cloudfront_distribution.hiteshshop.hosted_zone_id
      }
    }
  ]
}
